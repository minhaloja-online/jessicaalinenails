...
function aplicarAparencia(ap){
  ...
  r.setProperty("--tinta-suave", misturar(ap.corTexto, ap.corFundo, .42));  // legível sobre o fundo
...
/* ------------------------------------------------------------
   TEXTOS
   ------------------------------------------------------------ */
/** "Jessica Aline Nails" — o nome como se escreve numa frase. */
const nomeDoEstudio = () => `${CFG.marca.nome1} ${CFG.marca.nome2}`.trim();

function aplicarTextos(cfg){
  cfg.marca.nomeCompleto = `${cfg.marca.nome1} <i>${cfg.marca.nome2}</i>`;
  cfg.marca.nomeSimples  = `${cfg.marca.nome1} ${cfg.marca.nome2}`.trim();
  $$("[data-txt]").forEach(el => {
  ...
  if(assinatura) assinatura.textContent = cfg.marca.nomeSimples;
...
function aplicarContato(c){
  ...
  $$("[data-insta]").forEach(el => {
    if(c.instagram) { abrir(el, c.instagram); el.hidden = false; }
    else el.hidden = true;                 // sem Instagram, o botão não aparece
  });
...
async function enviarPedido(){
  ...
  const texto =
    `Olá, ${AG.prof.nome}! Quero confirmar um horário pelo site do ${nomeDoEstudio()}:\n\n` +
...
function montarEquipe(resolver = (r) => (r?.startsWith("midia:") ? "" : r || "")){
  ...
  alvo.innerHTML = time.map((p, i) => {
    ...
    <article class="prof rev" data-atraso="${i % 4}">
      <div class="moldura grao ${url ? "tem-foto" : ""}">
        <div class="foto" style="${url ? `background-image:url('${url.replace(/'/g, "%27")}')` : ""}"></div>
        <div class="marca"><b>${escapar((p.nome || "JA").slice(0, 2).toUpperCase())}</b><small>Foto</small></div>
      </div>
...
function montarServicos(resolver = (r) => (r?.startsWith("midia:") ? "" : r || "")){
  ...
  if(!itens.length){
    alvo.innerHTML = `<div class="vazio">Nenhum serviço cadastrado ainda.</div>`;
    return;
  }
  ...
  alvo.innerHTML = itens.map((s, i) => {
    ...
    return `
    <article class="serv rev" data-atraso="${i % 3}" data-previa="${url ? escapar(url) : ""}">
      <div class="nome-serv">
      ...
      <div class="fim-linha">
        ...
        <a class="marcar" href="#" data-agendar="${escapar(s.nome)}">Agendar
  ...
  ligarPrevia($$("#grade-serv .serv"));
}

/** Miniatura do serviço que acompanha o cursor sobre a fileira. */
function ligarPrevia(linhas){
  const previa = $("#previa");
  if(!previa || semMovimento || !podeHover) return;

  linhas.forEach(linha => {
    const url = linha.dataset.previa;
    if(!url) return;
    ...
    linha.addEventListener("pointermove", e => {
      previa.style.transform =
        `translate(${e.clientX - 100}px, ${e.clientY - 130}px) scale(1)`;
    });
    linha.addEventListener("pointerleave", () => previa.classList.remove("on"));
  });
}
  ...
  $("#mosaico").innerHTML = fotosGaleria.map((url, i) => `
    <button class="peca rev" data-atraso="${i % 4}" data-i="${i}" aria-label="Ampliar foto ${i + 1}">
      <div class="moldura grao ${url ? "tem-foto" : ""}">
        ...
        <div class="marca"><b>JA</b><small>Foto ${i + 1}</small></div>
      </div>
...
function montarDepoimentos(lista){
  ...
  alvo.innerHTML = lista.slice(0, 12).map((d, i) => `
    <article class="dep rev" data-atraso="${i % 3}">
      ...
      <div class="quem">
        ...
        <div><b>${escapar(d.nome)}</b><small>Cliente do estúdio</small></div>
      </div>
    </article>`).join("");
...
/* ------------------------------------------------------------
   Um único observador do rolar cuida de três coisas: o cabeçalho
   que encolhe, o fio de progresso da leitura e o leve deslocamento
   da foto de abertura. Tudo dentro de um requestAnimationFrame
   para não travar o rolar no celular.
   ------------------------------------------------------------ */
const topo    = $("#topo");
const fio     = $("#progresso-fio");
const fotoAlt = $(".hero-arte .moldura");
let agendado = false;

function aoRolar(){
  const y = scrollY;
  topo.classList.toggle("fixo", y > 40);

  if(fio){
    const total = document.documentElement.scrollHeight - innerHeight;
    fio.style.setProperty("--p", total > 0 ? Math.min(1, y / total).toFixed(4) : 0);
  }
  if(fotoAlt && !semMovimento) fotoAlt.style.transform = `translateY(${Math.min(y, 800) * .05}px)`;
  agendado = false;
}
aoRolar();
...
const vigia = new IntersectionObserver((ent) => {
  ent.forEach(e => {
    if(e.isIntersecting) botoes.forEach(b => b.classList.toggle("ativo", b.dataset.ir === e.target.id));
  });
}, { threshold: 0, rootMargin: "-45% 0px -45% 0px" });
$$("section[id]").forEach(sec => vigia.observe(sec));

/* brilho que acompanha o cursor na abertura */
...
const mostraFoto = (i) => {
  ...
  quadro.innerHTML = url ? "" : `<div style="display:grid;place-content:center;height:100%;font-family:var(--display);font-size:3rem;color:rgba(25,21,32,.3)">JA</div>`;
};
...
/* contagem de 00 a 100 enquanto a abertura está na tela */
(function contarAbertura(){
  const alvo = $("#carga-conta");
  if(!alvo) return;
  if(semMovimento){ alvo.textContent = "100"; return; }

  const inicio = performance.now(), duracao = 1400;
  const passo = (agora) => {
    const t = Math.min(1, (agora - inicio) / duracao);
    alvo.textContent = String(Math.round(t * 100)).padStart(2, "0");
...
setTimeout(liberarTela, semMovimento ? 200 : 1500);
